...
class AdsAbstract(models.Model):
    ...
    description = models.TextField(
        "Описание", help_text="Опишите вашего потерянного питомца"
    )
...
class Message(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["recipient", "checked"], name="recipient checked idx")
        ]

    def __str__(self):
        return (
            f"От {self.sender.first_name} к {self.recipient.first_name}: "
            f"{self.content[:30]}"
        )
