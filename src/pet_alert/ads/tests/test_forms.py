...
class AdsFormsTests(BaseTestCaseWithFixtures):
    ...
    def _test_set_password_and_activate_user_with_empty_password(self, user):
        ...
        guest_client.get(
            reverse(
                "users:registration_activate", kwargs={"activation_key": activation_key}
            ),
            follow=True,
        )
        ...
        activation_response = guest_client.post(
            reverse(
                "users:registration_activate", kwargs={"activation_key": "set-password"}
            ),
            data=form_data,
            follow=True,
        )
    ...
    def _test_add_advertisement_form_with_registration(
        ...
        self.assertTrue(
            isinstance(
                guest_response.context.get("reg_form"), CreationFormWithoutPassword
            )
        )
        self.assertTrue(isinstance(guest_response.context.get("ad_form"), model_form))

        guest_response = guest_client.post(
            add_advertisement_url, data=form_data, follow=True
        )
