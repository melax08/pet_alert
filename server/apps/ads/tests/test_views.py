...
class AdsViewTests(BaseTestCaseWithFixtures):
    ...
    def test_wrong_open_close_request(self):
        ...
        # For close ad:
        post_request_data = {"m": "broken", "ad_id": self.lost_open_active_ad.id}
        response = self.authorized_client.post(
            reverse("ads:close_ad"),
            data=json.dumps(post_request_data),
            content_type=self.FETCH_CONTENT_TYPE,
        ...
        self.assertTrue(Found.objects.get(pk=self.found_open_active_ad.id).open)

        # For open ad:
        post_request_data = {"m": "broken", "ad_id": self.lost_closed_inactive_ad.id}
        response = self.authorized_client.post(
            reverse("ads:open_ad"),
            data=json.dumps(post_request_data),
            content_type=self.FETCH_CONTENT_TYPE,
        ...
        self.assertEqual(response.status_code, HTTPStatus.BAD_REQUEST)
        self.assertFalse(Lost.objects.get(pk=self.lost_closed_inactive_ad.id).open)
        self.assertFalse(Found.objects.get(pk=self.found_closed_inactive_ad.id).open)
    ...
    def _test_context(self, response_obj, expected_obj):
        ...
        context_fields = {
            ...
            for attr in expected_obj.__dict__
            if attr != "_state"
        }
    ...
    def test_lost_list_context(self):
        ...
        self._test_context(response.context["page_obj"][0], self.lost_open_active_ad)
    ...
    def test_found_list_context(self):
        ...
        self._test_context(response.context["page_obj"][0], self.found_open_active_ad)

    def test_lost_detail_context(self):
        """Lost ad detail page has correct context."""
        response = self.authorized_client.get(
            reverse("ads:lost_detail", kwargs={"ad_id": self.lost_open_active_ad.id})
        )
        self._test_context(response.context.get("ad"), self.lost_open_active_ad)

    def test_found_detail_context(self):
        """Found ad detail page has correct context."""
        response = self.authorized_client.get(
            reverse("ads:found_detail", kwargs={"ad_id": self.found_open_active_ad.id})
        )
        self._test_context(response.context.get("ad"), self.found_open_active_ad)
    ...
    def test_profile_inactive_context(self):
        ...
        self._test_context(response.context["page_obj"][0], self.found_closed_inactive_ad)
    ...
    def test_dialog_list_context(self):
        ...
        self._test_context(response.context["chats"][0], self.dialog)
    ...
    def test_dialog_detail_context(self):
        ...
        self._test_context(response.context["messages"][1], self.message_2)
