...
from server.apps.ads.forms import FoundForm, LostForm
from server.apps.users.forms import CreationFormWithoutPassword
from server.apps.users.models import User
from server.apps.users.views import INTERNAL_SET_SESSION_TOKEN
...
class AdsFormsTests(BaseTestCaseWithFixtures):
    ...
    def test_creation_form_without_password_wrong_email_phone(self):
        ...
        form_data = {"first_name": "Геральт", "phone": "123", "email": "email"}

        form = CreationFormWithoutPassword(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertFormError(form, "email", errors="Введите правильный адрес электронной почты.")
        self.assertFormError(
            form,
            "phone",
            errors=(
                "Введите корректный номер телефона (например, 8 (301) "
    ...
    def test_creation_form_without_password_used_email(self):
        ...
        self.assertFormError(
            form,
            "email",
            errors="Пользователь с таким Адрес электронной почты уже существует.",
        )
    ...
    def _test_set_password_and_activate_user_with_empty_password(self, user):
        ...
        guest_client.get(
            reverse("users:registration_activate", kwargs={"activation_key": activation_key}),
            follow=True,
        )

        # Checks whether the client has the required parameter in the session
        # to set a new password
        self.assertTrue(guest_client.session.get(INTERNAL_SET_SESSION_TOKEN))
        ...
        activation_response = guest_client.post(
            reverse("users:registration_activate", kwargs={"activation_key": "set-password"}),
            data=form_data,
            follow=True,
        )
        ...
        self.assertRedirects(activation_response, reverse("users:set_password_done"))
    ...
    def _test_add_advertisement_form_with_registration(
        ...
        self.assertTrue(
            isinstance(guest_response.context.get("reg_form"), CreationFormWithoutPassword)
        )
        self.assertTrue(isinstance(guest_response.context.get("ad_form"), model_form))

        guest_response = guest_client.post(add_advertisement_url, data=form_data, follow=True)
        ...
        self.assertTrue(
            User.objects.filter(
                email=form_data["email"], first_name=form_data["first_name"]
            ).exists()
        )
        self.assertTrue(check_password("", new_user.password))
    ...
    def test_lost_form_with_registration(self):
        ...
        self._test_add_advertisement_form_with_registration(
            Lost, LostForm, reverse("ads:add_lost"), form_data
        )
    ...
    def test_found_form_with_registration(self):
        ...
        self._test_add_advertisement_form_with_registration(
            Found, FoundForm, reverse("ads:add_found"), form_data
        )

    def _test_add_advertisement_form(self, model, add_advertisement_url, form_data):
        """Authorized user can create an advertisement."""
        ads_count_before = model.objects.count()
        users_count_before = User.objects.count()

        response = self.authorized_client.post(add_advertisement_url, data=form_data, follow=True)
    ...
    def test_lost_add_advertisement_form(self):
        ...
        self._test_add_advertisement_form(Lost, reverse("ads:add_lost"), form_data)
    ...
    def test_found_add_advertisement_form(self):
        ...
        self._test_add_advertisement_form(Found, reverse("ads:add_found"), form_data)
