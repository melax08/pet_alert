...
from .fixtures import TEMP_MEDIA_ROOT, BaseApiTestCaseWithFixtures
...
class AdsApiTests(BaseApiTestCaseWithFixtures):
    def _check_response_status_codes(self, is_auth):
        ...
        and anonymous user."""
        urls = {
            reverse("api:animal_type-list"): (status.HTTP_200_OK, status.HTTP_200_OK),
            reverse("api:animal_type-detail", kwargs={"slug": self.animal_type.slug}): (
                status.HTTP_200_OK,
                status.HTTP_200_OK,
            ),
            reverse("api:lost-list"): (status.HTTP_200_OK, status.HTTP_200_OK),
            reverse("api:found-list"): (status.HTTP_200_OK, status.HTTP_200_OK),
            reverse("api:lost-detail", kwargs={"pk": self.lost_open_active_ad.id}): (
                status.HTTP_200_OK,
                status.HTTP_200_OK,
            ),
            reverse("api:found-detail", kwargs={"pk": self.found_open_active_ad.id}): (
                status.HTTP_200_OK,
                status.HTTP_200_OK,
            ),
            reverse("api:lost-detail", kwargs={"pk": self.lost_closed_active_ad.id}): (
                status.HTTP_404_NOT_FOUND,
                status.HTTP_200_OK,
            ),
            reverse("api:found-detail", kwargs={"pk": self.found_closed_active_ad.id}): (
                status.HTTP_404_NOT_FOUND,
                status.HTTP_200_OK,
            ),
            reverse("api:lost-detail", kwargs={"pk": self.lost_open_inactive_ad.id}): (
                status.HTTP_404_NOT_FOUND,
                status.HTTP_200_OK,
            ),
            reverse("api:found-detail", kwargs={"pk": self.found_open_inactive_ad.id}): (
                status.HTTP_404_NOT_FOUND,
                status.HTTP_200_OK,
            ),
            reverse("api:lost-detail", kwargs={"pk": self.lost_closed_inactive_ad.id}): (
                status.HTTP_404_NOT_FOUND,
                status.HTTP_200_OK,
            ),
            reverse("api:found-detail", kwargs={"pk": self.found_closed_inactive_ad.id}): (
                status.HTTP_404_NOT_FOUND,
                status.HTTP_200_OK,
            ),
        }
    ...
    def test_api_another_user_requests_to_open_active_ads(self):
        ...
        urls_codes_map = {
            reverse(
                "api:lost-detail", kwargs={"pk": self.lost_open_active_ad.id}
            ): status.HTTP_200_OK,
            reverse(
                "api:found-detail", kwargs={"pk": self.found_open_active_ad.id}
            ): status.HTTP_200_OK,
            reverse(
                "api:lost-detail", kwargs={"pk": self.lost_closed_active_ad.id}
            ): status.HTTP_404_NOT_FOUND,
            reverse(
                "api:found-detail", kwargs={"pk": self.found_closed_active_ad.id}
            ): status.HTTP_404_NOT_FOUND,
            reverse(
                "api:lost-detail", kwargs={"pk": self.lost_open_inactive_ad.id}
            ): status.HTTP_404_NOT_FOUND,
            reverse(
                "api:found-detail", kwargs={"pk": self.found_open_inactive_ad.id}
            ): status.HTTP_404_NOT_FOUND,
            reverse(
                "api:lost-detail", kwargs={"pk": self.lost_closed_inactive_ad.id}
            ): status.HTTP_404_NOT_FOUND,
            reverse(
                "api:found-detail", kwargs={"pk": self.found_closed_inactive_ad.id}
            ): status.HTTP_404_NOT_FOUND,
        }
    ...
    def test_api_create_animal_type(self):
        ...
        admin_response = self.admin_client.post(
            animal_type_url, animal_type_data, format=self.FILES_FORMAT
        )
        self.assertEqual(admin_response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(AnimalType.objects.count(), animal_types_count_before + 1)
        self.assertEqual(AnimalType.objects.last().slug, animal_type_data["slug"])
        ...
        self.assertEqual(AnimalType.objects.last().slug, "parrots")
    ...
    def test_api_patch_animal_type(self):
        ...
        animal_type_url = reverse("api:animal_type-detail", kwargs={"slug": animal_type.slug})
        animal_types_count = AnimalType.objects.count()

        # Admin can patch animal type
        admin_response = self.admin_client.patch(
            animal_type_url, {"slug": "monkeys"}, format=self.DEFAULT_FORMAT
        )
        self.assertEqual(admin_response.status_code, status.HTTP_200_OK)
        self.assertEqual(AnimalType.objects.get(pk=animal_type.id).slug, "monkeys")
        ...
        user_response = self.another_client.patch(
            animal_type_url, {"slug": "pigeons"}, format=self.DEFAULT_FORMAT
        )
        self.assertEqual(user_response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(AnimalType.objects.get(pk=animal_type.id).slug, "monkeys")
        self.assertEqual(animal_types_count, AnimalType.objects.count())
        ...
        anon_response = self.guest_client.patch(
            animal_type_url, {"slug": "tigers"}, format=self.DEFAULT_FORMAT
        )
        self.assertEqual(anon_response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(AnimalType.objects.get(pk=animal_type.id).slug, "monkeys")
        self.assertEqual(animal_types_count, AnimalType.objects.count())
    ...
    def _test_create_advertisement(self, url, request_data, model):
        ...
        guest_response = self.guest_client.post(url, request_data, format=self.FILES_FORMAT)
        ...
        self.assertNotEqual(model.objects.first().description, request_data["description"])

        # Authorized user can create an ad
        request_data["image"] = self.generate_image()
        user_response = self.author_client.post(url, request_data, format=self.FILES_FORMAT)
        ...
        self.assertEqual(model.objects.first().description, request_data["description"])
    ...
    def test_api_create_lost(self):
        ...
        self._test_create_advertisement(reverse("api:lost-list"), request_data, Lost)
    ...
    def test_api_create_found(self):
        ...
        self._test_create_advertisement(reverse("api:found-list"), request_data, Found)
    ...
    def _test_create_with_readonly_fields(self, url, model):
        ...
        self.author_client.post(url, request_data, format=self.DEFAULT_FORMAT)
    ...
    def test_create_with_readonly_fields_found(self):
        ...
        self._test_create_with_readonly_fields(reverse("api:found-list"), Found)
    ...
    def _test_api_patch(self, reverse_url, model):
        ...
        request_url = reverse(reverse_url, kwargs={"pk": new_advertisement.pk})
        advertisements_count = model.objects.count()

        # Anonymous can't modify an advertisement.
        new_description = "Аноним попытался изменить объявление"
        guest_response = self.guest_client.patch(request_url, {"description": new_description})
        ...
        author_response = self.author_client.patch(request_url, {"description": new_description})

        self.assertEqual(author_response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertNotEqual(model.objects.first().description, new_description)
        self.assertEqual(advertisements_count, model.objects.count())

        # Admin can modify an advertisement.
        new_description = "Админ изменил описание объявления"
        admin_response = self.admin_client.patch(request_url, {"description": new_description})
