...
from server.apps.ads.signals import post_save_advertisement
...
class BaseApiTestCaseWithFixtures(APITestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_admin = User.objects.create_superuser(email="admin@example.com", password="123123")
    ...
    def setUp(self):
        ...
        self.author_client.credentials(
            HTTP_AUTHORIZATION=f"{self.AUTH_TOKEN_PREFIX} {self.user_author_token}"
        )
        self.admin_client.credentials(
            HTTP_AUTHORIZATION=f"{self.AUTH_TOKEN_PREFIX} {self.user_admin_token}"
        )
        self.another_client.credentials(
            HTTP_AUTHORIZATION=f"{self.AUTH_TOKEN_PREFIX} {self.user_another_token}"
        )
